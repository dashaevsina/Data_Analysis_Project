...
# Clustering
The R algorithm `hclust` will be used for agglomerative (bottom-up) hierarchical clustering based on Euclidean distance. Hierarchical clustering is computationally inexpensive, as opposed to k-means clustering.
...
```{r}
groups_hierarchical <- hclust(distMatrix_Euclidean, method="ward.D")
...
hclust_average <- hclust(distMatrix_Euclidean, method = "average")
plot(hclust_average)
...
```{r}
hclust_centroid <- hclust(distMatrix_Euclidean, method = "centroid")
...
```{r}
hclust_mcquitty <- hclust(distMatrix_Euclidean, method = "mcquitty")
...
```{r}
divisive_hclust <- diana(distMatrix_Euclidean, stand=TRUE)
